...
namespace ESD.Controllers.Standard.Configuration
{
    ...
    public class RoleController : ControllerBase
    {
        ...
        [HttpGet("getMenuTree/{roleId}")]
        [PermissionAuthorization(PermissionConst.ROLE_READ)]
        public async Task<IActionResult> GetMenuTree(long roleId)
        {
            var token = Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
            var userRole = _jwtService.GetRolename(token);
            if (userRole.Contains(RoleConst.ROOT))
            {
                return Ok(await _menuService.GetAllTreeMenu(roleId, true));
            }
            else
            {
                return Ok(await _menuService.GetAllTreeMenu(roleId, false));
            }
        }
        ...
        [HttpPost("set-menu-permissions/{roleId}")]
        [PermissionAuthorization(PermissionConst.ROLE_UPDATE)]
        public async Task<IActionResult> SetMenuPermission(long roleId, List<RoleMenuDto> roleList)
        {
            var token = Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
            var createdBy = _jwtService.GetUserIdFromToken(token);
            var result = await _roleService.SetMenuPermission(roleList, createdBy, roleId);
